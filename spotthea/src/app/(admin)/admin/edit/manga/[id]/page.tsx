...
import { BookMarked, Camera, Clock3, Info, Plus, Save, Sparkles, Tag } from "lucide-react";
...
const SECTION_DESCRIPTIONS: Record<SectionId, string> = {
  title: "Ubah judul utama, slug otomatis, judul alternatif, dan synopsis.",
  metadata: "Perbarui author, artist, bahasa asli, status, rating, format publikasi, dan jadwal rilis.",
  tags: "Kelola Content Warning, Format, Genre, dan Theme secara praktis.",
  covers: "Ganti cover/banner via upload langsung atau URL.",
};

const defaultFormState: MangaEditFormState = {
  title: "",
  slug: "",
  altTitle: "",
  synopsis: "",
...
function mapMangaToFormState(manga: Manga): MangaEditFormState {
  const parsedPublishAt = manga.publishAt ? new Date(manga.publishAt) : null;
  const isScheduled = Boolean(parsedPublishAt && parsedPublishAt.getTime() > Date.now());
  ...
  return {
    ...
    year: Number.isFinite(manga.year) ? manga.year : new Date().getFullYear(),
    originalLanguage: manga.originalLanguage ?? "ja",
    status: manga.status ?? "ongoing",
    type: manga.type ?? "manga",
    contentRating: manga.contentRating ?? "safe",
    publishMode: isScheduled ? "scheduled" : "immediate",
    scheduledAt: isScheduled ? toDateTimeLocalValue(manga.publishAt) : "",
    contentWarnings: normalizeTagList(manga.contentWarnings ?? []),
    formats: normalizeTagList(manga.formats ?? []),
    genres: normalizeTagList(manga.genres ?? []),
    themes: normalizeTagList(manga.themes ?? []),
...
export default function AdminEditMangaPage() {
  ...
  const [formState, setFormState] = useState<MangaEditFormState>(defaultFormState);
  ...
  const [hasLoadedManga, setHasLoadedManga] = useState(false);
  ...
  useEffect(() => {
    if (!token || !canWrite || !mangaId) {
      return;
    }

    setIsBootstrapping(true);
    setIsLoadingTaxonomy(true);
    setHasLoadedManga(false);
    setPageError(null);

    void Promise.all([
      fetchAdminMangaDetail(mangaId, token),
      fetchAdminGenres(token),
      fetchAdminThemes(token),
    ])
      .then(([detail, genres, themes]) => {
        setGenresOptions(genres);
        setThemesOptions(themes);
        setFormState(mapMangaToFormState(detail.manga));
        setEditingTitle(detail.manga.title);
        setSlugManuallyEdited(detail.manga.slug !== slugify(detail.manga.title));
        setHasLoadedManga(true);
      })
      .catch((error) => {
        ...
        setPageError("Gagal memuat data manga untuk diedit.");
      })
      .finally(() => {
        setIsLoadingTaxonomy(false);
        setIsBootstrapping(false);
      });
  }, [canWrite, mangaId, token]);
  ...
  const submit = async () => {
    if (!token || !canWrite || !mangaId) {
      setPageError("Hanya admin yang dapat mengedit manga.");
      return;
    }
    ...
    if (formState.publishMode === "scheduled" && Number.isNaN(new Date(publishAt).getTime())) {
      setPageError("Format tanggal jadwal rilis tidak valid.");
      return;
    }
    ...
    try {
      ...
      if (error instanceof ApiClientError) {
        ...
        setPageError("Gagal menyimpan perubahan manga.");
      }
    } finally {
  ...
  if (!token) {
    return <EmptyState title="Login dibutuhkan" description="Silakan login sebagai admin untuk mengedit manga." ctaHref="/login" ctaLabel="Masuk" />;
  }

  if (!canWrite) {
    return <EmptyState title="Akses ditolak" description="Halaman edit manga hanya untuk role admin." ctaHref="/admin/manga" ctaLabel="Kembali" />;
  }

  if (!mangaId) {
    return <EmptyState title="Manga tidak valid" description="ID manga tidak ditemukan pada URL." ctaHref="/admin/manga" ctaLabel="Kembali ke daftar manga" />;
  }

  if (isBootstrapping) {
    return (
      <section className="space-y-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
        <p className="text-sm text-[var(--text-secondary)]">Memuat data manga...</p>
      </section>
    );
  }
  ...
  if (!hasLoadedManga) {
    return (
      <EmptyState
        title="Manga tidak dapat dimuat"
        description={pageError ?? "Data manga gagal dimuat atau tidak ditemukan."}
        ctaHref="/admin/manga"
        ctaLabel="Kembali ke daftar manga"
      />
    );
  }
  ...
  return (
    <section className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm text-[var(--text-muted)]">Admin / Edit / Manga / {mangaId}</p>
          <h1 className="font-display text-3xl text-[var(--text-primary)]">Edit Manga</h1>
          <p className="mt-1 text-xs text-[var(--text-muted)]">
            Step {sectionIndex + 1}/{SECTION_ITEMS.length}: {activeSectionMeta.label} · {SECTION_DESCRIPTIONS[activeSection]}
          </p>
          {editingTitle ? <p className="mt-1 text-sm text-[var(--text-secondary)]">Editing: {editingTitle}</p> : null}
        </div>
